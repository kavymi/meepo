...
For `brew install kavymi/tap/meepo` to work, you need a separate GitHub repo at `github.com/kavymi/homebrew-tap`.
...
```bash
# On GitHub: create a new public repo named "homebrew-tap" under the kavymi org/user
# Or via gh CLI:
gh repo create kavymi/homebrew-tap --public --description "Homebrew tap for Meepo"
```

### 2. Seed it with the formula

```bash
git clone https://github.com/kavymi/homebrew-tap.git
cd homebrew-tap
mkdir -p Formula
...
Homebrew formulae for [Meepo](https://github.com/kavymi/meepo).
...
2. Create a **Fine-grained token** scoped to the `kavymi/homebrew-tap` repo
...
```bash
# Via gh CLI:
gh secret set HOMEBREW_TAP_TOKEN --repo kavymi/meepo
# Paste the token when prompted
```

Or: GitHub → kavymi/meepo → Settings → Secrets and variables → Actions → New repository secret → Name: `HOMEBREW_TAP_TOKEN`
...
3. `update-homebrew` job:
   ...
   - Clones `kavymi/homebrew-tap`, copies the updated formula, pushes
